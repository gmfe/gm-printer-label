    ...
    return (
      <div>
        <div>
          {
            defaultTempList && _.size(defaultTempList)
              ? <div>
                {i18next.t('载入模板：')}
                <Select style={{ width: '190px' }} value={editStore.tempKey} onChange={this.handleInsertTemp}>
        ...
        <Flex alignCenter className='gm-padding-top-5'>
          ...
          <input className='gm-printer-label-edit-input-custom' type='text' value={editStore.config.name} onChange={this.handlePageName}/>
        </Flex>
        <Flex alignCenter className='gm-padding-top-5'>
          <div>{i18next.t('标签尺寸')}：</div>
          <Select style={{ width: '190px' }} className='gm-printer-label-edit-select' value={editStore.config.page.type} onChange={this.handlePageType}>
