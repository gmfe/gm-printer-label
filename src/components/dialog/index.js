      ...
      <div>
        ...
        <div className='gm-modal gm-animated gm-animated-fade-in-bottom gm-dialog gm-dialog-confirm'>
          <div className='gm-modal-dialog'>
            <button onClick={() => { Dialog.hide() }} type='button' className='close gm-modal-close'>
              <span>×</span>
            </button>
...
Dialog.render = ({ title, children, onOK, onCancel }) => {
  ReactDOM.render(<Dialog title={title} onOK={onOK} onCancel={onCancel}>{children}</Dialog>, window.shadowRoot.getElementById('gm-printer-label-modal'))
}

Dialog.hide = () => {
  ReactDOM.unmountComponentAtNode(window.shadowRoot.getElementById('gm-printer-label-modal'))
}
