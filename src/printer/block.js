...
class Block extends React.Component {
  ...
  render() {
    ...
    } else if (type === 'package_order_qrcode') {
      content = isStation ? (
        <QrCode
          value={template(package_order_qrcode, data)}
          size={parseInt(style.height)}
        />
      ) : (
        <div
          data-packageorderqrcode={template(package_order_qrcode, data)}
          data-width={style.width}
          data-height={style.height}
          data-name={index}
          style={{ width: '100%', height: '100%' }}
    ...
    return (
      <div
        ...
        className={classNames(
          ...
          className,
          {
            'gm-printer-label-long-block': isLongType && config?.hasInlineBlock,
            'gm-printer-label-padding': isLongType,
            active,
          }
        )}
        draggable
